# Review Prompt 3.2 - Edge Case Hunter

Rolle: Edge Case Hunter

Auftrag:
- Reviewe den untenstehenden Diff gegen das reale Projekt.
- Du darfst Projektdateien lesen.
- Suche nur nach unbehandelten Randfaellen, Luecken in den Tests, versteckten Verhaltensnischen oder Scope-Drift.
- Melde nur echte Findings.
- Wenn keine Findings vorliegen, antworte exakt mit `Keine Findings.`

Wichtige Projektkontexte:
- Zeitfensterlogik sitzt in `src/CustomMaintenanceBundle/Service/StatusService.php`
- Formate muessen `d.m.Y` und `H:i` bleiben
- `3.2` ist als Parity-/Regression-Story gedacht, nicht als neues Feature
